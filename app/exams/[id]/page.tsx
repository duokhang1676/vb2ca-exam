...
import { EXAM_SPECS, questionTypeLabel, sectionModeLabel } from "@/lib/exam/constants";
    ...
    <div className="mx-auto max-w-3xl space-y-6">
      <div className="space-y-2">
        ...
        <p className="text-sm text-muted-foreground">
          {sectionModeLabel(sectionMode)}. Kiểm tra đề đã trích đúng chưa. Câu
          hỏi và đáp án sẽ được đảo khi bắt đầu làm bài
          {sectionMode !== "part1"
            ? `. Phần 2 có ${questions.length} câu${spec ? ` (định mức ${spec.total})` : ""}, ${clusterCount} cụm thông tin/tình huống${spec ? ` (định mức ${spec.clusters})` : ""}`
            : ""}
          .
        </p>
        {belowSpec && sectionMode !== "part1" ? (
        ...
        {belowClusters && sectionMode !== "part1" ? (
          <p className="rounded-lg bg-primary/10 px-3 py-2 text-sm text-primary">
      ...
      <StartExamButton examId={exam.id} sectionMode={sectionMode} />
    </div>
  );
}
